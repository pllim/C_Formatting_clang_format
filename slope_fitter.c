...
static int compute_integration_segments(
...
static void get_pixel_ramp_integration(
...
static void get_pixel_ramp_integration_segments_and_pedestal(
...
static int ols_slope_fit_pixels(
...
static double ramp_fit_pixel_rnoise_chargeloss_segs(
    struct ramp_data *rd, struct pixel_ramp *pr, struct segment_list *segs, npy_intp integ);

static void ramp_fit_pixel_rnoise_chargeloss_remove(
...
static int ramp_fit_pixel_integration_fit_slope_seg(
    struct simple_ll_node *current, struct ramp_data *rd, struct pixel_ramp *pr, npy_intp integ,
    int segnum);

static int ramp_fit_pixel_integration_fit_slope_seg_default(
    struct ramp_data *rd, struct pixel_ramp *pr, struct simple_ll_node *seg, npy_intp integ,
    int segnum);

static int ramp_fit_pixel_integration_fit_slope_seg_len1(
    struct ramp_data *rd, struct pixel_ramp *pr, struct simple_ll_node *seg, npy_intp integ,
    int segnum);

static int ramp_fit_pixel_integration_fit_slope_seg_len2(
    struct ramp_data *rd, struct pixel_ramp *pr, struct simple_ll_node *seg, npy_intp integ,
    int segnum);

static void ramp_fit_pixel_integration_fit_slope_seg_default_weighted(
    struct ramp_data *rd, struct pixel_ramp *pr, struct simple_ll_node *seg, npy_intp integ,
    int segnum, real_t power);

static void ramp_fit_pixel_integration_fit_slope_seg_default_weighted_ols(
...
static void print_segment_opt_res(
...
static inline npy_intp get_cube_index(
    struct ramp_data *rd, npy_intp integ, npy_intp row, npy_intp col)
{
 ...
 * MAIN
 */
static PyObject *ols_slope_fitter(
 ...
 */
static int add_segment_to_list(
 ...
 */
static void clean_segment_list(
    npy_intp nints,            /* The number of integrations */
 ...
 */
static int create_rate_product(
 ...
 */
static int create_rateint_product(
 ...
 */
static int get_ramp_data_arrays(
 ...
 * Also, validate the types for each array and get endianness functions.
 */
static void get_ramp_data_meta(
 ...
 */
static int get_ramp_data_parse(
    PyObject **Py_ramp_data, /* The RampData class */
    struct ramp_data *rd,    /* The ramp data */
 ...
 * MEDIAN RATE
 */
static int compute_median_rate(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr) /* The pixel ramp data */
{
    if (1 == rd->ngroups) {
 ...
 */
static int median_rate_1ngroup(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr) /* The pixel ramp data */
{
    npy_intp idx, integ;
 ...
 */
static int median_rate_default(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr) /* The pixel ramp data */
{
    int ret = 0;
...
static int median_rate_integration_sort_cmp(
 ...
 */
static void print_ramp_data_types(
    struct ramp_data *rd, /* The ramp data */
 ...
 * ramp_fit_class.
 */
static int py_ramp_data_get_int(
    PyObject *rd,     /* The RampData class */
    const char *attr) /* The attribute to get from the class */
{
    PyObject *Obj;
 ...
 */
static int ramp_fit_pixel_rnoise_chargeloss(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr) /* The pixel ramp data */
{
    int ret = 0;
 ...
 */
static int ramp_fit_pixel_integration(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr, /* The pixel ramp data */
    npy_intp integ)        /* The integration number */
{
 ...
 */
static real_t segment_len1_timing(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr, /* The pixel ramp data */
    npy_intp integ)        /* The current integration */
{
 ...
 */
static real_t segment_rnoise_len1(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 */
static real_t segment_rnoise_len2(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr) /* The pixel ramp data */
{
    real_t rnum, rden;
 ...
 */
static real_t segment_rnoise_default(
    struct ramp_data *rd,  /* The ramp data */
    struct pixel_ramp *pr, /* The pixel ramp data */
