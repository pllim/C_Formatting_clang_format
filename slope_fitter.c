...
/*                            Worker Functions                               */
/* ------------------------------------------------------------------------- */
static int add_segment_to_list(struct segment_list *segs, npy_intp start, npy_intp end);

static PyObject *build_opt_res(struct ramp_data *rd);

static void clean_opt_res(struct opt_res_product *opt_res);

static void clean_pixel_ramp(struct pixel_ramp *pr);

static void clean_ramp_data(struct ramp_data *pr);

static void clean_rate_product(struct rate_product *rate_prod);

static void clean_rateint_product(struct rateint_product *rateint_prod);

static void clean_segment_list(npy_intp nints, struct segment_list *segs);

static void clean_segment_list_basic(struct segment_list *segs);

static int compute_integration_segments(struct ramp_data *rd, struct pixel_ramp *pr, struct segment_list *segs,
                                        int chargeloss, npy_intp integ);

static int cr_list_add(struct cr_list *crs, real_t crmag);

static void cr_list_clean(struct cr_list *crs);

static int create_opt_res(struct opt_res_product *opt_res, struct ramp_data *rd);

static struct pixel_ramp *create_pixel_ramp(struct ramp_data *rd);

static int create_rate_product(struct rate_product *rate_prod, struct ramp_data *rd);

static int create_rateint_product(struct rateint_product *rateint_prod, struct ramp_data *rd);

static float get_float2(PyArrayObject *obj, npy_intp row, npy_intp col);

static float get_float4(PyArrayObject *obj, npy_intp integ, npy_intp group, npy_intp row, npy_intp col);

static float get_float3(PyArrayObject *obj, npy_intp integ, npy_intp row, npy_intp col);

static uint32_t get_uint32_2(PyArrayObject *obj, npy_intp row, npy_intp col);

static void get_pixel_ramp(struct pixel_ramp *pr, struct ramp_data *rd, npy_intp row, npy_intp col);

static void get_pixel_ramp_integration(struct pixel_ramp *pr, struct ramp_data *rd, npy_intp row, npy_intp col,
                                       npy_intp integ, npy_intp group, npy_intp idx);

static void get_pixel_ramp_meta(struct pixel_ramp *pr, struct ramp_data *rd, npy_intp row, npy_intp col);

static void get_pixel_ramp_zero(struct pixel_ramp *pr);

static void get_pixel_ramp_integration_segments_and_pedestal(npy_intp integ, struct pixel_ramp *pr,
                                                             struct ramp_data *rd);

static struct ramp_data *get_ramp_data(PyObject *args);

static int get_ramp_data_arrays(PyObject *Py_ramp_data, struct ramp_data *rd);

static void get_ramp_data_meta(PyObject *Py_ramp_data, struct ramp_data *rd);

static int get_ramp_data_parse(PyObject **Py_ramp_data, struct ramp_data *rd, PyObject *args);

static int get_ramp_data_new_validate(struct ramp_data *rd);

static void get_ramp_data_dimensions(struct ramp_data *rd);

static void get_ramp_data_getters(struct ramp_data *rd);

static int compute_median_rate(struct ramp_data *rd, struct pixel_ramp *pr);

static int median_rate_1ngroup(struct ramp_data *rd, struct pixel_ramp *pr);

static int median_rate_default(struct ramp_data *rd, struct pixel_ramp *pr);

static real_t *median_rate_get_data(real_t *data, npy_intp integ, struct ramp_data *rd, struct pixel_ramp *pr);

static uint8_t *median_rate_get_dq(uint8_t *data, npy_intp integ, struct ramp_data *rd, struct pixel_ramp *pr);

static int median_rate_integration(real_t *mrate, real_t *int_data, uint8_t *int_dq, struct ramp_data *rd,
                                   struct pixel_ramp *pr);

static int median_rate_integration_sort(real_t *loc_integ, uint8_t *int_dq, struct ramp_data *rd,
                                        struct pixel_ramp *pr);

static int median_rate_integration_sort_cmp(const void *aa, const void *bb);

static int ols_slope_fit_pixels(struct ramp_data *rd, struct pixel_ramp *pr, struct rate_product *rate_prod,
                                struct rateint_product *rateint_prod);

static PyObject *package_results(struct rate_product *rate, struct rateint_product *rateints, struct ramp_data *rd);

static void prune_segment_list(struct segment_list *segs);

static float py_ramp_data_get_float(PyObject *rd, const char *attr);

static int py_ramp_data_get_int(PyObject *rd, const char *attr);

static int ramp_fit_pixel(struct ramp_data *rd, struct pixel_ramp *pr);

static int ramp_fit_pixel_rnoise_chargeloss(struct ramp_data *rd, struct pixel_ramp *pr);

static double ramp_fit_pixel_rnoise_chargeloss_segs(struct ramp_data *rd, struct pixel_ramp *pr,
                                                    struct segment_list *segs, npy_intp integ);

static void ramp_fit_pixel_rnoise_chargeloss_remove(struct ramp_data *rd, struct pixel_ramp *pr, npy_intp integ);

static int ramp_fit_pixel_integration(struct ramp_data *rd, struct pixel_ramp *pr, npy_intp integ);

static int ramp_fit_pixel_integration_fit_slope(struct ramp_data *rd, struct pixel_ramp *pr, npy_intp integ);

static int ramp_fit_pixel_integration_fit_slope_seg(struct simple_ll_node *current, struct ramp_data *rd,
                                                    struct pixel_ramp *pr, npy_intp integ, int segnum);

static int ramp_fit_pixel_integration_fit_slope_seg_default(struct ramp_data *rd, struct pixel_ramp *pr,
                                                            struct simple_ll_node *seg, npy_intp integ, int segnum);

static int ramp_fit_pixel_integration_fit_slope_seg_len1(struct ramp_data *rd, struct pixel_ramp *pr,
                                                         struct simple_ll_node *seg, npy_intp integ, int segnum);

static int ramp_fit_pixel_integration_fit_slope_seg_len2(struct ramp_data *rd, struct pixel_ramp *pr,
                                                         struct simple_ll_node *seg, npy_intp integ, int segnum);

static void ramp_fit_pixel_integration_fit_slope_seg_default_weighted(struct ramp_data *rd, struct pixel_ramp *pr,
                                                                      struct simple_ll_node *seg, npy_intp integ,
                                                                      int segnum, real_t power);

static void ramp_fit_pixel_integration_fit_slope_seg_default_weighted_ols(struct ramp_data *rd, struct pixel_ramp *pr,
                                                                          struct simple_ll_node *seg,
                                                                          struct ols_calcs *ols, npy_intp integ,
                                                                          int segnum, real_t power);

static void ramp_fit_pixel_integration_fit_slope_seg_default_weighted_seg(struct ramp_data *rd, struct pixel_ramp *pr,
                                                                          struct simple_ll_node *seg,
                                                                          struct ols_calcs *ols, npy_intp integ,
                                                                          int segnum, real_t power);

static real_t real_nan_median(real_t *arr, npy_intp len);

static int save_opt_res(struct opt_res_product *opt_res, struct ramp_data *rd);

static int save_ramp_fit(struct rateint_product *rateint_prod, struct rate_product *rate_prod, struct pixel_ramp *pr);

static real_t segment_len1_timing(struct ramp_data *rd, struct pixel_ramp *pr, npy_intp integ);

static real_t segment_rnoise_default(struct ramp_data *rd, struct pixel_ramp *pr, real_t seglen);

static real_t segment_rnoise_len1(struct ramp_data *rd, struct pixel_ramp *pr, real_t timing);

static real_t segment_rnoise_len2(struct ramp_data *rd, struct pixel_ramp *pr);

static int segment_snr(real_t *snr, npy_intp integ, struct ramp_data *rd, struct pixel_ramp *pr,
                       struct simple_ll_node *seg, int segnum);

static real_t snr_power(real_t snr);
/* ------------------------------------------------------------------------- */

/* ------------------------------------------------------------------------- */
/*                            Debug Functions                                */
/* ------------------------------------------------------------------------- */
static void print_cr_pixel(struct pixel_ramp *pr, int line);

static void print_cr_pixel_integ(struct pixel_ramp *pr, struct cr_list *crs, npy_intp integ, int line);

static void print_real_array(char *label, real_t *arr, int len, int ret, int line);

static void print_intp_array(npy_intp *arr, int len, int ret);

static void print_npy_types();

static void print_ols_calcs(struct ols_calcs *ols, npy_intp integ, int segnum, int line);

static void print_pixel_ramp_data(struct ramp_data *rd, struct pixel_ramp *pr, int line);

static void print_pixel_ramp_dq(struct ramp_data *rd, struct pixel_ramp *pr, int line);

static void print_pixel_ramp_info(struct ramp_data *rd, struct pixel_ramp *pr, int line);

static void print_pixel_ramp_stats(struct pixel_ramp *pr, int line);

static void print_PyArrayObject_info(PyArrayObject *obj);

static void print_ramp_data_info(struct ramp_data *rd);

static void print_ramp_data_types(struct ramp_data *rd, int line);

static void print_rd_type_info(struct ramp_data *rd);

static void print_segment_list(npy_intp nints, struct segment_list *segs, int line);

static void print_segment_list_basic(struct segment_list *segs, int line);

static void print_segment_list_integ(npy_intp integ, struct segment_list *segs, int line);

static void print_segment(struct simple_ll_node *seg, struct ramp_data *rd, struct pixel_ramp *pr, npy_intp integ,
                          int segnum, int line);

static void print_segment_opt_res(struct simple_ll_node *seg, struct ramp_data *rd, npy_intp integ, int segnum,
                                  int line);

static void print_stats(struct pixel_ramp *pr, npy_intp integ, int line);

static void print_uint8_array(uint8_t *arr, int len, int ret, int line);

static void print_uint32_array(uint32_t *arr, int len, int ret, int line);
/* ========================================================================= */
...
static inline npy_intp get_ramp_index(struct ramp_data *rd, npy_intp integ, npy_intp group) {
    return rd->ngroups * integ + group;
}

/* Translate 3-D (integ, row, col) to a 1-D index. */
static inline npy_intp get_cube_index(struct ramp_data *rd, npy_intp integ, npy_intp row, npy_intp col) {
    return rd->image_sz * integ + rd->ncols * row + col;
}

/* Print a line delimiter for visual separation.  Used for debugging. */
static inline void print_delim() {
    int k;
    const char c = '-';
    for (k = 0; k < 80; ++k) {
        printf("%c", c);
...
/* Print a line delimiter for visual separation.  Used for debugging. */
static inline void print_delim_char(char c, int len) {
    int k;
    for (k = 0; k < len; ++k) {
        printf("%c", c);
    }
 ...
 */
static inline int is_pix_in_list(struct ramp_data *rd, struct pixel_ramp *pr) {
    /* Pixel list */
    // JP-3669 - (1804, 173)
    const int len = 1;
    npy_intp rows[len];
...
static inline long long print_pid_info(long long prev, int line, char *label) {
 ...
 * MAIN
 */
static PyObject *ols_slope_fitter(PyObject *module, /* The ramp fitting module for the C extension. */
 ...
 */
static int add_segment_to_list(struct segment_list *segs, /* The list to add the segment to. */
 ...
 */
static PyObject *build_opt_res(struct ramp_data *rd) /* The ramp fitting data */
{
    ...
    opt_res_info = Py_BuildValue("NNNNNNNNN", opt_res.slope, opt_res.sigslope, opt_res.var_p, opt_res.var_r,
                                 opt_res.yint, opt_res.sigyint, opt_res.pedestal, opt_res.weights, opt_res.cr_mag);
 ...
 */
static void clean_pixel_ramp(struct pixel_ramp *pr) /* Ramp fitting data for a pixel. */
{
    npy_intp integ;
...
static void clean_ramp_data(struct ramp_data *rd) /* The ramp fitting data structure */
{
    npy_intp idx;
 ...
 */
static void clean_rate_product(struct rate_product *rate_prod) /* Rate product data structure */
{
    /* Free all arrays */
 ...
 */
static void clean_rateint_product(struct rateint_product *rateint_prod) /* Rateints product data structure */
{
    /* Free all arrays */
 ...
 */
static void clean_segment_list(npy_intp nints, /* The number of integrations */
 ...
 * out all elements.
 */
static void clean_segment_list_basic(struct segment_list *segs) /* The segment list to clean */
{
    struct simple_ll_node *current = NULL;
    struct simple_ll_node *next = NULL;
 ...
 */
static int compute_integration_segments(struct ramp_data *rd, /* Ramp fitting data */
 ...
 * the last.
 */
static int cr_list_add(struct cr_list *crs, /* Cosmic ray list for integration ramp. */
 ...
 */
static int create_opt_res(struct opt_res_product *opt_res, /* The optional results product */
 ...
 */
static struct pixel_ramp *create_pixel_ramp(struct ramp_data *rd) /* The ramp fitting data */
{
    ...
    if ((NULL == pr->data) || (NULL == pr->groupdq) || (NULL == pr->rateints) || (NULL == pr->segs) ||
        (NULL == pr->stats) || (NULL == pr->is_zframe) || (NULL == pr->is_0th) || (NULL == pr->crs)) {
        snprintf(msg, 255, "Couldn't allocate memory for pixel ramp data structure.");
        PyErr_SetString(PyExc_MemoryError, (const char *)msg);
        err_ols_print("%s\n", msg);
        FREE_PIXEL_RAMP(pr);
 ...
 */
static int create_rate_product(struct rate_product *rate, /* The rate product */
                               struct ramp_data *rd) /* The ramp fitting data */
{
    const npy_intp nd = 2;
    npy_intp dims[nd];
 ...
 */
static int create_rateint_product(struct rateint_product *rateint, /* The rateints product */
                                  struct ramp_data *rd) /* The ramp fitting data */
{
    const npy_intp nd = 3;
    npy_intp dims[nd];
 ...
 */
static real_t real_nan_median(real_t *arr, /* Array in which to find the median */
...
static float get_float2(PyArrayObject *obj, /* Object from which to get float */
...
static float get_float4(PyArrayObject *obj, /* Object from which to get float */
...
static float get_float3(PyArrayObject *obj, /* Object from which to get float */
                        npy_intp integ, /* Integration index into object */
                        npy_intp row, /* Row index into object */
                        npy_intp col) /* Column index into object */
{
...
static uint32_t get_uint32_2(PyArrayObject *obj, /* Object from which to get float */
                             npy_intp row, /* Row index into object */
                             npy_intp col) /* Column index into object */
{
 ...
 */
static void get_pixel_ramp(struct pixel_ramp *pr, /* Pixel ramp data */
 ...
 */
static void get_pixel_ramp_integration(struct pixel_ramp *pr, /* Pixel ramp data */
                                       struct ramp_data *rd, /* Ramp data */
 ...
 */
static void get_pixel_ramp_meta(struct pixel_ramp *pr, /* Pixel ramp data */
                                struct ramp_data *rd, /* Ramp data */
                                npy_intp row, /* Pixel row */
                                npy_intp col) /* Pixel column */
{
 ...
 * for the next pixel.
 */
static void get_pixel_ramp_zero(struct pixel_ramp *pr) /* Pixel ramp data */
{
    pr->pixeldq = 0.;
    pr->gain = 0.;
    pr->rnoise = 0.;
 ...
 */
static void get_pixel_ramp_integration_segments_and_pedestal(npy_intp integ, /* The current integration */
 ...
 */
static struct ramp_data *get_ramp_data(PyObject *args) /* The C extension module arguments */
{
 ...
 */
static int get_ramp_data_arrays(PyObject *Py_ramp_data, /* The inputted RampData */
                                struct ramp_data *rd) /* The ramp data */
{
    /* Get numpy arrays */
 ...
 * Also, validate the types for each array and get endianness functions.
 */
static void get_ramp_data_meta(PyObject *Py_ramp_data, /* The RampData class */
                               struct ramp_data *rd) /* The ramp data */
{
 ...
 */
static int get_ramp_data_parse(PyObject **Py_ramp_data, /* The RampData class */
                               struct ramp_data *rd, /* The ramp data */
                               PyObject *args) /* The C extension module arguments */
{
    ...
    if (!PyArg_ParseTuple(args, "OOOsI:get_ramp_data", Py_ramp_data, &(rd->gain), &(rd->rnoise), &weight,
                          &rd->save_opt)) {
 ...
 */
static int get_ramp_data_new_validate(struct ramp_data *rd) /* the ramp data */
{
    char *msg = NULL;
 ...
 */
static void get_ramp_data_dimensions(struct ramp_data *rd) /* The ramp data */
{
    npy_intp *dims;
 ...
 */
static void get_ramp_data_getters(struct ramp_data *rd) /* The ramp data */
{
 ...
 * MEDIAN RATE
 */
static int compute_median_rate(struct ramp_data *rd, /* The ramp data */
                               struct pixel_ramp *pr) /* The pixel ramp data */
{
    if (1 == rd->ngroups) {
 ...
 */
static int median_rate_1ngroup(struct ramp_data *rd, /* The ramp data */
                               struct pixel_ramp *pr) /* The pixel ramp data */
{
    npy_intp idx, integ;
 ...
 */
static int median_rate_default(struct ramp_data *rd, /* The ramp data */
                               struct pixel_ramp *pr) /* The pixel ramp data */
{
    int ret = 0;
 ...
 */
static real_t *median_rate_get_data(real_t *data, /* Integration data */
 ...
 */
static uint8_t *median_rate_get_dq(uint8_t *data, /* Integration data quality */
                                   npy_intp integ, /* The integration number */
                                   struct ramp_data *rd, /* The ramp data */
                                   struct pixel_ramp *pr) /* The pixel ramp data */
{
 ...
 */
static int median_rate_integration(real_t *mrate, /* The NaN median rate */
 ...
 * Sort the modified data.
 */
static int median_rate_integration_sort(real_t *loc_integ, /* Local copy of integration data */
                                        uint8_t *int_dq, /* The integration data quality */
                                        struct ramp_data *rd, /* The ramp data */
                                        struct pixel_ramp *pr) /* The pixel ramp data */
{
...
static int median_rate_integration_sort_cmp(const void *aa, /* First comparison element */
 ...
 */
static int ols_slope_fit_pixels(struct ramp_data *rd, /* The ramp data */
                                struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 */
static void print_ramp_data_types(struct ramp_data *rd, /* The ramp data */
 ...
 */
static PyObject *package_results(struct rate_product *rate, /* The rate product */
                                 ...
                                 struct ramp_data *rd) /* The ramp data */
{
    ...
    image_info = Py_BuildValue("(NNNNN)", rate->slope, rate->dq, rate->var_poisson, rate->var_rnoise, rate->var_err);
    if (!image_info) {
        goto FAILED_ALLOC;
    }

    cube_info = Py_BuildValue("(NNNNN)", rateints->slope, rateints->dq, rateints->var_poisson, rateints->var_rnoise,
                              rateints->var_err);
    if (!cube_info) {
        goto FAILED_ALLOC;
    }
 ...
 */
static void print_rd_type_info(struct ramp_data *rd) { /* The ramp data */
    print_delim();
    print_npy_types();
 ...
 */
static void prune_segment_list(struct segment_list *segs) /* Linked list of segments */
{
    struct simple_ll_node *seg = NULL;
 ...
 * ramp_fit_class.
 */
static float py_ramp_data_get_float(PyObject *rd, /* The RampData class */
 ...
 * ramp_fit_class.
 */
static int py_ramp_data_get_int(PyObject *rd, /* The RampData class */
                                const char *attr) /* The attribute to get from the class */
{
    PyObject *Obj;
    int val;
 ...
 * PIXEL RAMP
 */
static int ramp_fit_pixel(struct ramp_data *rd, /* The ramp data */
                          struct pixel_ramp *pr) /* The pixel ramp data */
{
    int ret = 0;
    npy_intp integ;
 ...
 */
static int ramp_fit_pixel_rnoise_chargeloss(struct ramp_data *rd, /* The ramp data */
                                            struct pixel_ramp *pr) /* The pixel ramp data */
{
    int ret = 0;
    int is_chargeloss = 0;
 ...
 */
static void ramp_fit_pixel_rnoise_chargeloss_remove(struct ramp_data *rd, /* The ramp data */
                                                    struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 */
static int ramp_fit_pixel_integration(struct ramp_data *rd, /* The ramp data */
                                      struct pixel_ramp *pr, /* The pixel ramp data */
                                      npy_intp integ) /* The integration number */
{
    int ret = 0;
...
#define DBG_SEG_ID                                                                                                     \
    do {                                                                                                               \
        dbg_ols_print("   *** [Integ: %ld] (%ld, %ld) Seg: %d, Length: %ld, Start: %ld, End: %ld\n", integ, pr->row,   \
                      pr->col, segcnt, current->length, current->start, current->end);                                 \
    } while (0)
 ...
 */
static int ramp_fit_pixel_integration_fit_slope(struct ramp_data *rd, /* The ramp data */
                                                struct pixel_ramp *pr, /* The pixel ramp data */
                                                npy_intp integ) /* The integration number */
{
    int ret = 0;
 ...
 */
static int ramp_fit_pixel_integration_fit_slope_seg(struct simple_ll_node *current, /* The current segment */
                                                    struct ramp_data *rd, /* The ramp data */
                                                    struct pixel_ramp *pr, /* The pixel ramp data */
                                                    npy_intp integ, /* The integration number */
 ...
 */
static int ramp_fit_pixel_integration_fit_slope_seg_default(struct ramp_data *rd, /* The ramp data */
                                                            struct pixel_ramp *pr, /* The pixel ramp data */
                                                            ...
                                                            int segnum) /* Teh segment number */
{
    ...
    if (WEIGHTED == rd->weight) {
        ramp_fit_pixel_integration_fit_slope_seg_default_weighted(rd, pr, seg, integ, segnum, power);
    } else {
 ...
 * segment of length 1.
 */
static int ramp_fit_pixel_integration_fit_slope_seg_len1(struct ramp_data *rd, /* The ramp data */
                                                         struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 */
static real_t segment_len1_timing(struct ramp_data *rd, /* The ramp data */
                                  struct pixel_ramp *pr, /* The pixel ramp data */
                                  npy_intp integ) /* The current integration */
{
    if (!rd->suppress1g) {
 ...
 */
static real_t segment_rnoise_len1(struct ramp_data *rd, /* The ramp data */
                                  struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 * segment of length 2.
 */
static int ramp_fit_pixel_integration_fit_slope_seg_len2(struct ramp_data *rd, /* The ramp data */
                                                         struct pixel_ramp *pr, /* The pixel ramp data */
                                                         struct simple_ll_node *seg, /* The integration segment */
                                                         npy_intp integ, /* The integration number */
                                                         int segnum) /* The segment number */
 ...
 */
static real_t segment_rnoise_len2(struct ramp_data *rd, /* The ramp data */
                                  struct pixel_ramp *pr) /* The pixel ramp data */
{
    real_t rnum, rden;
 ...
 */
static void
ramp_fit_pixel_integration_fit_slope_seg_default_weighted(struct ramp_data *rd, /* The ramp data */
                                                          struct pixel_ramp *pr, /* The pixel ramp data */
                                                          ...
                                                          real_t power) /* The power of the segment */
{
    ...
    ramp_fit_pixel_integration_fit_slope_seg_default_weighted_ols(rd, pr, seg, &ols, integ, segnum, power);

    /* From weighted OLS variables fit segment. */
    ramp_fit_pixel_integration_fit_slope_seg_default_weighted_seg(rd, pr, seg, &ols, integ, segnum, power);
}
 ...
 */
static void
ramp_fit_pixel_integration_fit_slope_seg_default_weighted_ols(struct ramp_data *rd, /* The ramp data */
                                                              struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 */
static void
ramp_fit_pixel_integration_fit_slope_seg_default_weighted_seg(struct ramp_data *rd, /* The ramp data */
                                                              struct pixel_ramp *pr, /* The pixel ramp data */
                                                              struct simple_ll_node *seg, /* The integration segment */
                                                              struct ols_calcs *ols, /* Intermediate calculations */
                                                              npy_intp integ, /* The integration number */
                                                              ...
                                                              real_t power) /* The power of the segment */
{
    ...
    real_t sumx = ols->sumx, sumxx = ols->sumxx, sumy = ols->sumy, sumxy = ols->sumxy, sumw = ols->sumw;
 ...
 */
static real_t segment_rnoise_default(struct ramp_data *rd, /* The ramp data */
                                     struct pixel_ramp *pr, /* The pixel ramp data */
 ...
 */
static int save_opt_res(struct opt_res_product *opt_res, /* The optional results product */
                        struct ramp_data *rd) /* The ramp data */
{
    void *ptr = NULL;
 ...
 */
static int save_ramp_fit(struct rateint_product *rateint_prod, /* The rateints product */
                         struct rate_product *rate_prod, /* The rate product */
                         struct pixel_ramp *pr) /* The pixel ramp data */
{
    void *ptr = NULL;
...
static int segment_snr(real_t *snr, /* The signal to noise ratio for a segment */
...
static real_t snr_power(real_t snr) /* The signal to noise ratio of a segment */
{
    if (snr < 5.) {
        return 0.;
    } else if (snr < 10.) {
...
static void print_segment_list(npy_intp nints, struct segment_list *segs, int line) {
    ...
    for (integ = 0; integ < nints; ++integ) {
        ...
        printf("%sSegments of Integration [%ld] has %ld segment(s), with max length %ld.\n", indent, integ,
               segs[integ].size, segs[integ].max_segment_length);
        while (current) {
            next = current->flink;
...
PyMODINIT_FUNC PyInit_slope_fitter(void) {
    PyObject *m;
